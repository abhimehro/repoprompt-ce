...
actor MCPConfigExportService {
    ...
    func prepareStableWrapperConfigFile() throws -> URL {
        ...
        let tempURL = configDirectoryURL.appendingPathComponent("temp-\(UUID().uuidString).json", isDirectory: false)
    ...
    private func makeLease(prefix: String, contents: String) throws -> MCPConfigLease {
        ...
        guard fileManager.createFile(atPath: url.path, contents: data, attributes: [.posixPermissions: 0o400]) else {
            throw CocoaError(.fileWriteUnknown)
        }
