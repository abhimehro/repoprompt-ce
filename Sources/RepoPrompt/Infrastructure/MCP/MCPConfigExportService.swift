...
actor MCPConfigExportService {
    ...
    func prepareStableWrapperConfigFile() throws -> URL {
        ...
        guard let data = configJSON.data(using: .utf8) else { throw CocoaError(.fileWriteUnknown) }
        let tempURL = configDirectoryURL.appendingPathComponent(".\(identity.stableWrapperConfigFileName).\(UUID().uuidString).tmp")
        ...
        if fileManager.fileExists(atPath: configURL.path) {
            _ = try fileManager.replaceItem(at: configURL, withItemAt: tempURL, backupItemName: nil, options: [])
        } else {
    ...
    private func makeLease(prefix: String, contents: String) throws -> MCPConfigLease {
        ...
        guard let data = contents.data(using: .utf8) else { throw CocoaError(.fileWriteUnknown) }
        let tempURL = launchConfigDirectoryURL.appendingPathComponent(".\(url.lastPathComponent).\(UUID().uuidString).tmp")
        guard fileManager.createFile(atPath: tempURL.path, contents: data, attributes: [.posixPermissions: 0o400]) else {
            throw CocoaError(.fileWriteUnknown)
        }
        defer { try? fileManager.removeItem(at: tempURL) }
        if fileManager.fileExists(atPath: url.path) {
            _ = try fileManager.replaceItem(at: url, withItemAt: tempURL, backupItemName: nil, options: [])
        } else {
