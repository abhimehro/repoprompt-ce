...
actor MCPConfigExportService {
    ...
    func prepareStableWrapperConfigFile() throws -> URL {
        ...
        if !fileManager.createFile(atPath: tempURL.path, contents: data, attributes: [.posixPermissions: 0o600]) {
            throw CocoaError(.fileWriteUnknown)
        }
        if fileManager.fileExists(atPath: configURL.path) {
            _ = try fileManager.replaceItemAt(configURL, withItemAt: tempURL, backupItemName: nil, options: .usingNewMetadataOnly)
        } else {
    ...
    private func makeLease(prefix: String, contents: String) throws -> MCPConfigLease {
        ...
        if !fileManager.createFile(atPath: tempURL.path, contents: data, attributes: [.posixPermissions: 0o400]) {
            throw CocoaError(.fileWriteUnknown)
        }
