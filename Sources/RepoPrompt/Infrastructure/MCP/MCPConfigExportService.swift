...
actor MCPConfigExportService {
    ...
    func prepareStableWrapperConfigFile() throws -> URL {
        ...
        let tempURL = configDirectoryURL.appendingPathComponent(".tmp-\(UUID().uuidString)", isDirectory: false)
        guard fileManager.createFile(atPath: tempURL.path, contents: configJSON.data(using: .utf8), attributes: [.posixPermissions: 0o600]) else {
    ...
    private func makeLease(prefix: String, contents: String) throws -> MCPConfigLease {
        ...
        let tempURL = launchConfigDirectoryURL.appendingPathComponent(".tmp-\(UUID().uuidString)", isDirectory: false)
        guard fileManager.createFile(atPath: tempURL.path, contents: contents.data(using: .utf8), attributes: [.posixPermissions: 0o400]) else {
            throw CocoaError(.fileWriteUnknown)
        }
        defer { try? fileManager.removeItem(at: tempURL) }
