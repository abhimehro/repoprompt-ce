...
public enum MCPTerminalRecordStore {
    ...
    public static func write(
        ...
        guard fileManager.createFile(atPath: tempURL.path, contents: try encoder.encode(record), attributes: [.posixPermissions: 0o600]) else {
            throw CocoaError(.fileWriteUnknown)
        }

        if fileManager.fileExists(atPath: fileURL.path) {
            _ = try fileManager.replaceItem(at: fileURL, withItemAt: tempURL, backupItemName: nil, options: [])
        } else {
