...
def parse_identity_output(
    output: str, certificate_name: str
...
def collect_inventory(fixture: dict[str, Any], *, now: datetime) -> dict[str, Any]:
    ...
    all_identities = parse_identity_output(
        str(fixture.get("all_identity_output", "")), certificate_name
    )
    valid_identities = parse_identity_output(
        str(fixture.get("valid_identity_output", "")), certificate_name
    )
    ...
    exact_certificates = [
        ...
        if certificate["subject_common_name"] == certificate_name
    ]
    ...
    return {
        ...
        "summary": {
            ...
            "private_key_backed_identity_count": sum(
                bool(certificate["private_key_backed"])
                for certificate in exact_certificates
            ),
            "valid_private_key_backed_identity_count": sum(
                bool(certificate["valid_code_signing_identity"])
                for certificate in exact_certificates
            ),
            "certificate_without_private_key_count": sum(
                not bool(certificate["private_key_backed"])
                for certificate in exact_certificates
            ),
...
def main() -> int:
    ...
    parser.add_argument(
        "--fixture",
        required=True,
        help="Captured JSON input; this tool never reads a Keychain",
    )
    parser.add_argument(
        "--at",
        help="Evaluation timestamp; defaults to fixture evaluated_at or current UTC time",
    )
    ...
    now = (
        parse_iso_datetime(str(evaluation_value))
        if evaluation_value
