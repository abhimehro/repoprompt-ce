...
def timeline(
    connection: str, invocation: str, tool: str, start: float, run_id: str | None = None
) -> list[dict]:
    success = (
...
class CaptureParsingTests(unittest.TestCase):
    def test_build_samples_uses_event_completion_main_actor_exit_envelope(self) -> None:
        ...
        self.assertEqual(
            sample["stages"][-1]["dimensions"]["observerType"], "event_completion"
        )
    ...
    def test_build_samples_rejects_missing_envelope_endpoint(self) -> None:
        ...
        with self.assertRaisesRegex(
            benchmark.BenchmarkError, "event_completion MainActorExited"
        ):
    ...
    def test_build_samples_requires_run_id_on_event_completion(self) -> None:
        ...
        ] = "tool=read_file observerType=event_completion"
        with self.assertRaisesRegex(
            benchmark.BenchmarkError, "event_completion omitted runID"
        ):
            benchmark.build_samples({"lifecycle_events": events})
...
class RoutingTests(unittest.TestCase):
    def test_opposite_tool_order_signatures_classify_local_and_worktree(self) -> None:
        ...
        for index, tool in enumerate(["read_file", "file_search", "file_search"]):
            events += timeline(
                "connection-b", f"b-{index}", tool, offset, "worktree-run"
            )
            offset += 10
    ...
    def test_unexpected_or_extra_connection_fails_classification(self) -> None:
        samples = benchmark.build_samples(
            {"lifecycle_events": timeline("only", "one", "file_search", 0)}
        )
        with self.assertRaisesRegex(
            benchmark.BenchmarkError, "two file-tool connections"
        ):
    ...
    def test_classification_rejects_capture_from_unexpected_agent_run(self) -> None:
        ...
        events += timeline(
            "connection-b", "b-search", "file_search", 30, "worktree-run"
        )
        samples = benchmark.build_samples({"lifecycle_events": events})
    ...
    def test_nested_json_payload_schema_is_found(self) -> None:
        wrapped = {
            "content": [
                {
                    "text": json.dumps(
                        {"ok": True, "op": "capture", "capture": {"active": False}}
                    )
                }
            ]
        }
        self.assertEqual(
            benchmark.find_object(wrapped, "capture", "capture")["capture"],
            {"active": False},
        )
...
class IntegrityAndCleanupTests(unittest.TestCase):
    def test_transcript_arguments_require_exact_workload(self) -> None:
        ...
        with self.assertRaisesRegex(
            benchmark.BenchmarkError, "exact configured workload"
        ):
    ...
    def test_transcript_rejects_extra_tools_and_wrong_read_values(self) -> None:
        ...
        with self.assertRaisesRegex(
            benchmark.BenchmarkError, "unexpected transcript tools"
        ):
            benchmark.validate_transcript_arguments(transcript, "KNOWN", path, 24)
    ...
    def test_compacted_transcript_allows_key_paths_and_single_set_status(self) -> None:
        ...
        status = (
            '<tool_call name="set_status">{"session_name":"diagnostic"}</tool_call>'
        )
        compacted_workload = (
            f'<tool_call name="read_file">{json.dumps(read)}</tool_call>\n'
            f'<tool_call name="file_search">{json.dumps(search)}</tool_call>'
    ...
    def test_transcript_rejects_unknown_read_enrichment_and_missing_surface(
        self,
    ) -> None:
        path = "Sources/Marker.swift"
        search = {
    ...
    def test_route_binding_metadata_independently_proves_local_and_worktree_routes(
        self,
    ) -> None:
        ...
        results = {
            ...
            "worktree": {
                ...
                "wait_response": {
                    "status": "completed",
                    "worktree": {
                        "path": "/tmp/rpce-route-worktree/../rpce-route-worktree"
                    },
                },
            },
        }
        self.assertEqual(
            benchmark.validate_route_binding_metadata(results, configured), []
        )

        results["worktree"]["wait_response"] = {"status": "completed"}
        failures = benchmark.validate_route_binding_metadata(results, configured)
        self.assertTrue(
            any("omitted worktree binding" in failure for failure in failures)
        )

        results["worktree"]["wait_response"] = {
            "worktree": {"path": "/tmp/wrong-worktree"}
        }
        ...
        failures = benchmark.validate_route_binding_metadata(results, configured)
        self.assertTrue(
            any("local agent unexpectedly" in failure for failure in failures)
        )
    ...
    def test_agent_completion_requires_completed_status_and_exact_token(self) -> None:
        ...
        results = {
            "local": {
                "status": "completed",
                "wait_response": {"assistant_text": "AGENT_FILE_TOOL_DIAGNOSTIC_OK"},
            },
        ...
        results["local"]["wait_response"] = {
            "assistant_text": "almost AGENT_FILE_TOOL_DIAGNOSTIC_OK"
        }
    ...
    def test_agent_session_identity_requires_matching_distinct_start_and_wait_sessions(
        self,
    ) -> None:
        results = {
            "local": {
        ...
        self.assertTrue(
            any("worktree agent start/wait" in failure for failure in failures)
        )
        results["worktree"]["wait_response"] = {
            "session_id": "worktree-session",
    ...
    def test_agent_run_identity_rejects_conflicting_aliases(self) -> None:
        ...
        with self.assertRaisesRegex(
            benchmark.BenchmarkError, "local agent wait response omitted run_id"
        ):
            benchmark.expected_agent_run_ids(results)

    def test_positive_search_stages_require_local_and_worktree_projection_counts(
        self,
    ) -> None:
        capture = {
            "stages": [
        ...
        self.assertEqual(
            benchmark.positive_search_projection_counts(capture), {False: 4}
        )

    def test_sequential_calls_reject_same_connection_overlap_but_allow_cross_agent_overlap(
        self,
    ) -> None:
        samples = [
            {
                ...
                "main_actor_exited_offset_ms": 5,
            },
            {
                "connection_id": "a",
                "join_key": "a2",
                "received_offset_ms": 4,
                "main_actor_exited_offset_ms": 7,
            },
            {
                "connection_id": "b",
                "join_key": "b1",
                "received_offset_ms": 1,
                "main_actor_exited_offset_ms": 6,
            },
        ]
    ...
    def test_cancel_failure_and_nonterminal_settle_are_cleanup_failures(self) -> None:
        ...
        failures = benchmark.cancellation_failure_messages(
            "local", cancel_error, "running"
        )
        self.assertEqual(len(failures), 2)
        self.assertIn("cancellation failed", failures[0])
        self.assertIn("remained nonterminal", failures[1])
        self.assertEqual(
            benchmark.cancellation_failure_messages("local", None, "cancelled"), []
        )

    def test_cleanup_removes_only_owned_clean_terminal_worktree(self) -> None:
        self.assertEqual(
            benchmark.cleanup_decision(True, True, True, True),
            "remove_clean_auto_created",
        )
        self.assertEqual(
            benchmark.cleanup_decision(True, True, True, False),
            "preserve_nonterminal_sessions",
        )
        self.assertEqual(
            benchmark.cleanup_decision(True, True, False, True), "preserve_dirty"
        )
        self.assertEqual(
            benchmark.cleanup_decision(False, True, True, False), "preserve_not_owned"
        )
        self.assertEqual(
            benchmark.cleanup_decision(True, False, True, False), "already_absent"
        )

    def test_worktree_identity_requires_same_common_dir_link_and_distinct_root(
        self,
    ) -> None:
        common = Path("/tmp/repo/.git").resolve()
        benchmark.validate_worktree_identity(
            common, common, linked=True, same_as_workspace_root=False
        )
        with self.assertRaisesRegex(benchmark.BenchmarkError, "different git common"):
            benchmark.validate_worktree_identity(
                common, Path("/tmp/other/.git"), True, False
            )
...
class ReplayFixtureTests(unittest.TestCase):
    def test_success_fixture_replays_exact_scrubbed_timings_offline(self) -> None:
        ...
        self.assertEqual(
            summary["samples"]["sample_count"], fixture["expected"]["sample_count"]
        )
        samples = benchmark.build_samples(fixture["capture"])
        actual_timings = {
            sample["join_key"]: sample["envelope_ms"] for sample in samples
        }
        self.assertEqual(actual_timings, fixture["expected"]["envelope_ms"])
        grouped = {
            (group["route"], group["tool"]): group["p50_ms"]
            for group in summary["samples"]["groups"]
        }
        self.assertEqual(grouped[("local", "file_search")], 105.353)
        self.assertEqual(grouped[("worktree", "read_file")], 22.929)

        with mock.patch.object(
            benchmark, "resolve_cli", side_effect=AssertionError("live CLI used")
        ):
            with mock.patch.object(
                benchmark, "command", side_effect=AssertionError("subprocess used")
            ):
    ...
    def test_success_fixture_rejects_missing_worktree_projection(self) -> None:
        fixture = json.loads(
            (FIXTURE_ROOT / "paired-success" / "replay.json").read_text(
                encoding="utf-8"
            )
        )
        ...
        self.assertTrue(
            any(
                "local/worktree projections" in failure
                for failure in summary["integrity"]["failures"]
            )
        )
        ...
        for event_value in fixture["capture"]["lifecycle_events"]:
            if event_value.get("event_name") == "Search.ProviderDTOReady":
                event_value["sanitized_dimensions"] = re.sub(
                    r"matchCount=\d+",
                    "matchCount=0",
                    event_value["sanitized_dimensions"],
                )
                break
        with mock.patch.object(benchmark, "load_replay_input", return_value=fixture):
            summary = benchmark.replay_artifact(Path("fixture.json"))
    ...
    def test_checked_in_fixture_text_is_privacy_scrubbed(self) -> None:
        ...
        for replay_file in sorted(FIXTURE_ROOT.glob("*/replay.json")):
            ...
            for agent in fixture["agents"].values():
                ...
                assistant = re.findall(
                    r"<assistant>(.*?)</assistant>", transcript, re.DOTALL
                )
