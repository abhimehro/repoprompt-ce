...
final class ToolGroupsTests: XCTestCase {
    ...
    func testParseGroups_WithAliases() throws {
        ...
        XCTAssertEqual(groups, [.binding, .context, .conversation]) // canonical order mapping aliases
    }
    ...
    func testFilterTools() {
        let tool1 = MCP.Tool(name: "bind_context", description: "", inputSchema: .init(type: "object", properties: [:]))
        let tool2 = MCP.Tool(name: "git", description: "", inputSchema: .init(type: "object", properties: [:]))
        let tool3 = MCP.Tool(name: "unknown_tool", description: "", inputSchema: .init(type: "object", properties: [:]))
    ...
    func testIsInGroups() {
        ...
        XCTAssertTrue(ToolGroupCatalog.isInGroups("git", groups: [.explore])) // In both git and explore
